...
st.markdown(f"<p style='text-align:center; color:#004C97; font-size:14px;'>🕒 Ultimo aggiornamento: {oggi} – Versione 1.1</p>", unsafe_allow_html=True)
...
if file_att and file_tab:
    ...
    # 4️⃣ Matching intelligente (fuzzy)
    def find_best_match(name, lista):
        if not name or not lista:
            return None
        best = max(lista, key=lambda x: SequenceMatcher(None, name, x).ratio(), default=None)
        ratio = SequenceMatcher(None, name, best).ratio() if best else 0
        return best if ratio >= 0.7 else None

    righe_output = []
    clienti_att = att["NomeSoggetto_n"].dropna().unique().tolist()
    ...
    for _, r in tab.iterrows():
        ...
        nome_matchato = ""
        if att_cli.empty and cliente_norm:
            best_match = find_best_match(cliente_norm, clienti_att)
            if best_match:
                att_cli = att[att["NomeSoggetto_n"] == best_match]
                nome_matchato = best_match
        ...
        if not att_cli.empty:
            ...
            righe_output.append({
                ...
                "Cliente": r["Cliente"],
                "Cliente abbinato (attività)": nome_matchato,
                "Anno": anno_att,
                "Mese": mese_att,
    ...
    my_bar.progress(60, text="📊 Match attività completato (fuzzy match attivo)...")
    ...
    my_bar.progress(75, text="🎨 Formattazione Excel in corso...")

    # 6️⃣ Scrivi Excel formattato
    output = BytesIO()
